...
Tools for simplifying traditional English spellings using CMU pronunciations. Includes:
- A browser-based live translator (`eng_to_us.html`).
- A CLI modernizer (`modernize.py`) that reads stdin and writes simplified text.

American English note: `american.txt` and `rules.json` are no longer used; everything now keys off `CMU.txt` (the CMU Pronouncing Dictionary) for deterministic phonemes.
...
You need a small HTTP server so the page can fetch `CMU.txt`.

- Linux/macOS:  
  ```bash
  python3 -m http.server 8000
  ```  
  Then open http://localhost:8000/eng_to_us.html in your browser.

- Windows (PowerShell):  
  ```powershell
  python -m http.server 8000
  ```  
  Then open http://localhost:8000/eng_to_us.html.
...
## Files
- `eng_to_us.html`: Browser translator; needs an HTTP server to fetch `CMU.txt`.
- `modernize.py`: CLI modernizer (stdin → stdout).
- `CMU.txt`: CMU Pronouncing Dictionary (primary dependency).  
